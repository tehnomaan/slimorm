...
public class Database {
	...
	public <T> T insert(T entity) throws Exception {
		...
		return runStatements((db, conn) -> {
			boolean hasAutoGeneratedId = (props.idField != null && !props.idField.isMutable);
			try(PreparedStatement stmt = conn.prepareStatement(props.sqlInsert + props.sqlInsertValues, hasAutoGeneratedId  ? new String[] {props.idField.columnName} : null)) {
				bindMutableParameters(0, entity, props, stmt);
				stmt.executeUpdate();
	...
	public <T> List<T> bulkInsert(List<T> entities) throws Exception {
		...
		return runStatements((db, conn) -> {
			...
			boolean hasAutoGeneratedId = (props.idField != null && !props.idField.isMutable);
			try(PreparedStatement stmt = conn.prepareStatement(sql, hasAutoGeneratedId  ? new String[] {props.idField.columnName} : null)) {
				int ordinal = 0;
				for(int i = 0; i < array.length; i++)
					ordinal = bindMutableParameters(ordinal, array[i], props, stmt);
				stmt.execute();
				if (hasAutoGeneratedId) {
					ResultSet rs = stmt.getGeneratedKeys();
					int rowIndex = 0;
	...
	public void update(Object entity, String whereExpression, Object ... whereParameters) throws Exception {
		runStatements((db, conn) -> {
			...
			try(PreparedStatement stmt = conn.prepareStatement(props.sqlUpdate + " WHERE " + whereExpression)) {
				int ordinal = bindMutableParameters(0, entity, props, stmt);
	...
	private <T> int bindMutableParameters(int ordinal, T entity, EntityProperties props, PreparedStatement stmt) throws SQLException, IllegalAccessException {
